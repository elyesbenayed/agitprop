...
NAME_OF = dict(DEPARTMENTS)

# Régions (groupes de comptes prédéfinis)
REGIONS = {
...
def render_caption(text: str, code: str, username: str | None = None) -> str:
    ...
    if not text:
        return text or ""
    return (text.replace("{departement}", NAME_OF.get(code, code))
                .replace("{département}", NAME_OF.get(code, code))
                .replace("{code}", code)
                .replace("{compte}", ("@" + username) if username else "")
                .replace("{region}", REGION_OF.get(code, ""))
                .replace("{région}", REGION_OF.get(code, "")))
