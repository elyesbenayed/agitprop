# Mise en ligne d'IG Manager chez Infomaniak
...
`igmanager` → IP du VPS (nom libre : `socials`, `reseaux`…). Attendre quelques minutes.

## 3. Envoyer les sources (depuis ce PC, PowerShell, dans le dossier IGManager)

```
ssh root@IP "mkdir -p /root/igmanager-src"
scp -r app templates static requirements.txt deploy root@IP:/root/igmanager-src/
```
...
```
ssh root@IP
cd /root/igmanager-src
bash deploy/install-vps.sh igmanager.lafrancehumaniste.fr votre@email.fr
```

Le script installe Python, nginx, le certificat HTTPS Let's Encrypt, le service systemd
(redémarrage automatique) et le pare-feu. À la fin : `https://igmanager.lafrancehumaniste.fr`.
Identifiants admin dans `/opt/igmanager/IDENTIFIANTS.txt` : à ranger dans le gestionnaire
de mots de passe, puis supprimer le fichier.

## 5. Ensuite

- Renseigner `META_APP_ID` et `META_APP_SECRET` dans `/opt/igmanager/.env`, puis `systemctl restart igmanager`.
- Mettre à jour l'app : refaire les étapes 3 et 4 (les données sont conservées).
- Sauvegarder régulièrement `/opt/igmanager/data/igmanager.db` et `/opt/igmanager/.env`.
- Journal : `journalctl -u igmanager -f`.
